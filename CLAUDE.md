...
- `bin/` — Bash deployment helpers for a Linux target host, not used in local dev: `install-deps.sh` provisions a fresh Debian box (nginx, nvm/node, pnpm, ufw firewall rules), `deploy.sh` does `git reset --hard origin/main` + `pnpm install` + build + copy `apps/web/dist` to a target directory (default `/var/www/html/home-dashboard`), served behind nginx. The frontend is a static build; only the backend runs as a long-lived Node process.
...
- `IsolarProvider` polls `isolar-solar-data` every 5 minutes while logged in and invalidates the login-status query on fetch errors (so an expired iSolarCloud session correctly falls back to the login dialog rather than silently failing).
