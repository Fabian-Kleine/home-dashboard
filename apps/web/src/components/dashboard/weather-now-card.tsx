...
export function WeatherNowCard({
  ...
  const metrics: { key: string; label: string; value: string; unit?: string; color: string; icon: TablerIcon }[] = [
    ...
    { key: "humidity", label: t.weatherNow.humidity, value: String(weather.relativeHumidity), unit: "%", color: "#16a99a", icon: IconDroplet },
    { key: "wind", label: t.weatherNow.wind, value: String(Math.round(weather.windSpeed)), unit: "km/h", color: "#6a72e8", icon: IconWind },
    { key: "precip", label: t.weatherNow.precip, value: weather.precipitation.toFixed(1), unit: "mm", color: "#0ea5e9", icon: IconUmbrella },
    { key: "sunrise", label: t.weatherNow.sunrise, value: formatTime(weather.sunrise), color: "#f2a52c", icon: IconSunrise },
    { key: "sunset", label: t.weatherNow.sunset, value: formatTime(weather.sunset), color: "#f2734f", icon: IconSunset },
  ];
